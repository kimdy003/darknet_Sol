...
void free_layer(layer l)
{
    ...
    if(l.type == CONVOLUTIONAL){
        if(l.weights)            cuda_freehost(l.weights);
        if(l.weight_updates)     cuda_freehost(l.weight_updates);
        if(l.biases)             cuda_freehost(l.biases);
        if(l.bias_updates)       cuda_freehost(l.bias_updates);
        if(l.delta)              cuda_freehost(l.delta);
        if(l.output)             cuda_freehost(l.output);
        if(l.binary_weights)     cuda_freehost(l.binary_weights);
        if(l.cweights)           cuda_freehost(l.cweights);
        if(l.scales)             cuda_freehost(l.scales);
        if(l.scale_updates)      cuda_freehost(l.scale_updates);
        if(l.mean)               cuda_freehost(l.mean);
        if(l.variance)           cuda_freehost(l.variance);
        if(l.mean_delta)         cuda_freehost(l.mean_delta);
        if(l.variance_delta)     cuda_freehost(l.variance_delta);
        if(l.rolling_mean)       cuda_freehost(l.rolling_mean);
        if(l.rolling_variance)   cuda_freehost(l.rolling_variance);
        if(l.x)                  cuda_freehost(l.x);
        if(l.x_norm)             cuda_freehost(l.x_norm);
        if(l.m)                  cuda_freehost(l.m);
        if(l.v)                  cuda_freehost(l.v);
    }
    else{
        if(l.weights)            free(l.weights);
        if(l.weight_updates)     free(l.weight_updates);
